...
import com.example.pasteleria1000sabores.data.remote.RetrofitClient
import com.example.pasteleria1000sabores.data.remote.dto.auth.LoginPasteleriaRequest
import com.example.pasteleria1000sabores.data.remote.dto.auth.RegisterPasteleriaRequest
import com.example.pasteleria1000sabores.model.User
import com.example.pasteleria1000sabores.model.ApiResult
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext

class AuthPasteleriaRepository {

    // Acceso directo a las dependencias desde RetrofitClient
    private val tokenManager = RetrofitClient.getTokenManager()
    private val apiService = RetrofitClient.pasteleriaAuthApiService
     ...
     */
    private fun saveSession(token: String, user: com.example.pasteleria1000sabores.data.remote.dto.auth.UserDto) {
    ...
    suspend fun login(email: String, password: String): ApiResult<User> = withContext(Dispatchers.IO) {
        try {
            val request = LoginPasteleriaRequest(email, password)
            ...
            if (response.isSuccessful) {
                ...
                if (body != null && body.success && body.data != null) {
                    ...
                    val user = User(
                        id = authData.user.id,
                        nombre = authData.user.nombre ?: "Sin nombre",
    ...
    suspend fun register(
        ...
        try {
            val request = RegisterPasteleriaRequest(
                email = email,
                password = password,
                role = role,
                nombre = nombre,
    ...
    suspend fun createProductorUser(
        ...
        try {
            val request = com.example.pasteleria1000sabores.data.remote.dto.auth.CreateProductorUserRequest(
                nombre = nombre,
                email = email,
                password = password,
                ubicacion = ubicacion,
        ...
        try {
            ...
            if (response.isSuccessful) {
                ...
                if (body != null && body.success && body.data != null) {
                    ...
                    val user = User(
                        id = userDto.id,
                        nombre = userDto.nombre ?: "Sin nombre",
                        email = userDto.email,
                        role = userDto.role,
